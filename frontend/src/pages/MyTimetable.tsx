...
export default function MyTimetable() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-[1200px] mx-auto pb-10">
      {/* Banner */}
      <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-r shadow-lg border ${isRep ? 'from-[#4A1115] via-[#4A1115] to-[#7B1D23] border-[#7B1D23]/30' : 'from-[#4A1115] via-[#632220] to-[#7B1D23] border-[#7B1D23]/30'}`}>
        ...
        <div className="absolute -bottom-24 -right-24 w-64 h-64 bg-[#3d0b0d] rounded-full blur-3xl opacity-50 pointer-events-none"></div>
        ...
        <div className="relative z-10 p-8 sm:p-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            <div className="p-3 bg-white/10 rounded-xl backdrop-blur-md border border-white/10 shadow-inner">
              <CalendarDays className="w-8 h-8 text-[#f2e6e4]" />
            </div>
            <div>
              <h1 className="text-3xl font-extrabold text-white tracking-tight drop-shadow-md">
                My Timetable
              </h1>
              <p className="text-[#f2e6e4]/80 font-medium mt-1 text-sm sm:text-base">
          ...
          <div className="bg-black/20 px-4 py-2 rounded-lg backdrop-blur-sm border border-white/5 text-right hidden sm:block shadow-inner">
            ...
            <p className="text-sm font-bold text-[#f2e6e4]">
              CSBS Department
            </p>
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-[#EAE0D9] shadow-sm space-y-4">
        <h2 className="text-xl font-bold text-[#4A1115] flex items-center gap-2 mb-2">
          Weekly Allocation
        </h2>
        
        {/* Render grid conditionally based on role */}
        {isRep ? (
          repClassId ? <TimetableGrid classId={repClassId} /> : <div className="p-8 text-center text-[#7B1D23]/60 font-medium">Locating your class schedule...</div>
        ) : (
